...
type SchoolCfg = { name: string; section: 'secondary' | 'university'; high1_cohort?: number; thresholds?: { before: number; after: number } }
const SCHOOLS: Record<string, SchoolCfg> = {
  TKB: { name: '筑波大学附属中学校・高等学校', section: 'secondary', high1_cohort: 136 },
  GAKU: { name: '学習院大学', section: 'university' }
}
const SECTION_DEFAULT = {
  secondary: { high1_cohort: 136, thresholds: { before: 3, after: -3 } },
  university: { thresholds: { before: -1, after: 4 } }
} as const
...
function convert(inputRaw: string): ConvertResult | null {
  ...
  if (section === 'secondary') {
    ...
    const admissionYear = num // 大学は入学年度
